...
public class ThreadFrdpNet implements Runnable
{
	...
	{	
		...
		try
		{
			socketR = new MulticastSocket(8102);
		...
		{
			AppLogger.error("FrdpNet绾跨▼鍒濆鍖栧け璐ワ紒", e);
			System.exit(1);
		}
		...
		while(true)
		{
			try
			{
				...
				{
				case 62:
					dispatchCat062(bufferR, 0, dpR.getLength());
					break;
				case 65:
			...
			{
				AppLogger.error("澶勭悊Frdp鏁版嵁澶辫触锛�", e);
			}	
		}						
	}
	
	public static void dispatchCat062(byte[] buf,int index,int len) throws IOException
	{
		TrackPoList tpl = App.getApp().getTrackPoList();
		
		ArrayList<TrackDb> tds = decodeCat062(buf, index, len);
		
		Gson gson=new GsonBuilder().serializeSpecialFloatingPointValues().create();
			
		BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:\\frdp_result.txt",true)));
		
		for(TrackDb td: tds) {
			tpl.updateData(td);	 
			
			String str_temp = gson.toJson(td);
	...
	public static ArrayList<TrackDb> decodeCat062(byte[] buf,int index,int len)
	{
...
//				System.out.println(speedx+" "+speedy);
			...
			if(bit390 == 1)
			{
				...
				if(bitCSN == 1)
				{
					String callsign_fpl = new String(buf, dataIndex, 7);
					if(!callsign_fpl.equals(""))
			...
			//System.out.println(ssr+" "+distancep+" "+speed+" "+speed_angle+" "+tracknum);
			//System.out.println("ssr = "+ssr);
			
			TrackDb td = new TrackDb();
			td.setTrackid(""+tracknum);
